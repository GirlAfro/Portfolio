import { Wifi, Server, Router, Smartphone, CheckCircle, ExternalLink } from 'lucide-react';

const Projects = () => {
  const projects = [
    {
      title: 'Comprehensive Network Troubleshooting Project',
        ...
        <div className="space-y-12">
          {projects.map((project, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`group bg-slate-900/50 backdrop-blur-sm border border-${project.color}-500/20 rounded-xl p-8 hover:border-${project.color}-500/40 transition-all duration-300`}
              >
                <div className="flex flex-col lg:flex-row gap-8">
                  {/* Project Header */}
                  <div className="flex-1">
                    <div className="flex items-start gap-4 mb-6">
                      <div className={`p-3 bg-${project.color}-500/10 rounded-lg`}>
                        <IconComponent className={`w-8 h-8 text-${project.color}-400`} />
                      </div>
                      <div className="flex-1">
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {project.technologies.map((tech, techIdx) => (
                          <span
                            key={techIdx}
                            className={`px-3 py-1 bg-${project.color}-500/10 text-${project.color}-400 rounded-full text-sm border border-${project.color}-500/20`}
                          >
                            {tech}
                          </span>
                        ))}
                    ...
                    <ul className="space-y-2">
                      {project.features.map((feature, featureIdx) => (
                        <li key={featureIdx} className="flex items-start gap-3">
                          <div className={`w-1.5 h-1.5 bg-${project.color}-400 rounded-full mt-2`}></div>
